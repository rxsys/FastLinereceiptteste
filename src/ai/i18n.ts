...
const TRANSLATIONS = {
  ...
  genericError:    { ja: '⚠️ [BUILD_17:05] 処理中にエラーが発生いたしました。\n[DEBUG]: Webhook do Next.js foi atingido.', pt: '⚠️ [BUILD_17:05] Erro de processamento.\n[DEBUG]: Webhook Next.js atingido.', en: '⚠️ [BUILD_17:05] Processing error.\n[DEBUG]: Next.js Webhook triggered.', es: '⚠️ [BUILD_17:05] Error de procesamiento.', zh: '⚠️ [BUILD_17:05] 发生错误。', ko: '⚠️ [BUILD_17:05] 오류가 발생했습니다.', vi: '⚠️ [BUILD_17:05] Đã xảy ra lỗi.' },
  ccRegistered: {
    ja: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ 登録が完了いたしました。\n📁 ${pName} / ${ccName} に記録いたしました。\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\n領収書は正常に受領され、現在確認（分析）を行っております。✨\n確認が完了しましたら改めてお知らせいたします。`,
    pt: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ Registrado!\n📁 Salvo em: ${pName} / ${ccName}.\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\nObrigado pelo envio! ✨\nSeu recibo foi registrado e agora passará por uma análise. Você receberá uma confirmação assim que for validado.`,
    en: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ Registered!\n📁 Saved to: ${pName} / ${ccName}.\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\nThank you for submission! ✨\nYour receipt will be verified, and you will receive a notification once confirmed.`,
    es: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ ¡Registrado!\n📁 Guardado en: ${pName} / ${ccName}.\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\n¡Gracias por el envío! ✨\nSu recibo será verificado y recibirá una notificación una vez confirmado.`,
    zh: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ 已登记！\n📁 保存到: ${pName} / ${ccName}。\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\n感谢提交！✨\n我们将验证您的收据，并在确认后通知您。`,
    ko: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ 등록 완료!\n📁 ${pName} / ${ccName}에 저장되었습니다.\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\n제출해 주셔서 감사합니다! ✨\n영수증을 확인 중이며, 확인이 완료되면 다시 알려드리겠습니다.`,
    vi: (ccName: string, amount: string, desc: string, date: string, pName: string) => `✅ Đã đăng ký!\n📁 Đã lưu vào: ${pName} / ${ccName}.\n💰 ¥${amount}\n📝 ${desc}\n📅 ${date}\n\nCảm ơn bạn đã gửi! ✨\nHóa đơn của bạn sẽ được xác minh và chúng tôi sẽ thông báo cho bạn sau khi xác nhận xong.`,
  } as TranslationEntry,
